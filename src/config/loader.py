...
class Config:
    ...
    def BYBIT_API_KEY(self):
        return self.get_env('BYBIT_API_KEY')

    @property
    def BYBIT_API_SECRET(self):
        return self.get_env('BYBIT_API_SECRET')

    @property
    def BOT_TOKEN_TELEGRAM(self):
        return self.get_env('BOT_TOKEN_TELEGRAM')

    @property
    def TELEGRAM_CHAT_ID(self):
        val = self.get_env('TELEGRAM_CHAT_ID')
        return str(val) if val is not None else None

    @property
    def BYBIT_DEFAULT_LEVERAGE(self):
        return int(self.get_config('bybit_futures', 'default_leverage', 10))

    @property
    def BYBIT_MAX_LEVERAGE(self):
        return int(self.get_config('bybit_futures', 'max_leverage', 50))

    @property
    def BYBIT_MIN_CONFIDENCE(self):
        return float(self.get_config('bybit_futures', 'min_confidence_to_trade', 55))
    ...
    def QUOTE_CURRENCY(self):
        """Extract quote currency from CRYPTO_PAIR (e.g., 'USDT' from 'BTC/USDT:USDT')."""
