...
class TemplateManager:
    ...
    def build_system_prompt(self, symbol: str, timeframe: str = "1h", previous_response: Optional[str] = None,
        ...
        header_lines.extend([
            ...
            "- CLOSE PROACTIVELY: If your analysis at candle close shows stalling momentum, signal CLOSE.",
            "- ⚠️ FEE AWARENESS (CRITICAL): Bybit charges ~0.12% round-trip fees. DO NOT signal CLOSE for 'micro-profits' (under 0.15% price difference) just because momentum stalled. If the gross profit is less than 0.15%, you are better off holding unless the thesis is completely invalidated and hitting SL is imminent.",
            "",
    ...
    def build_response_template(self, _has_chart_analysis: bool = False,
        ...
        # Reduced base confidence threshold to 50 (from 70) to prevent excessive hesitation
...
1. **MARKET STRUCTURE & DYNAMIC FORECAST**: Current phase and trend state. Include **Multi-Timeframe Summary**: list each available timeframe (4h/12h/24h/3d/7d/Weekly/365D) with directional bias → state ALIGNED/MIXED/DIVERGENT and identify dominant timeframe. **MANDATORY**: Explicitly predict the trajectory of the next 25m, 30m, 1h, 1d, 7d, and 30d candles based on your findings. Do not hesitate to guess the future shape of the market.
...
{{
    "analysis": {{
        ...
        "stop_loss": number,
        "take_profit": number,
        "position_size": 0.0-1.0, // DECIMAL PERCENTAGE OF TOTAL USDT CAPITAL (e.g. 0.3 = 30% of your total USDT). DO NOT output token amounts like BTC.
...
- Final position_size = max({min_pos_size:.2f}, calculated_value). This is a percentage of your total USDT capital.
- FORMAT: "Position: base [X] - alignment [Y] = [Z] (Decimal % of Total USDT)"
...
- HOLD (any confidence <{conf_threshold}): Mixed signals, weak trend, conflicting indicators, low volume.
- ANTI-HESITATION PROTOCOL: If you accurately predict market direction and momentum is turning, do NOT be afraid to fire a BUY/SELL command even if your confidence isn't perfect (e.g. 50-65 confidence). DO NOT wait for 3 consecutive red candles to confirm a short. Act proactively!
...
DYNAMIC TRAILING STOP-LOSS (UPDATE SIGNAL MANDATORY):
If a position is already highly profitable (e.g. Short at $1000, current price $900):
- DO NOT just hold and wait for TP blindly. Market structure can reverse.
- Signal `UPDATE` and move the Stop Loss deep into profit (e.g. to $940) based on new lower-highs/higher-lows to LOCK IN profits (Trailing Profit Protection).
- Continue to output `UPDATE` on subsequent checks to trail the SL tighter as price moves further in your favor.
